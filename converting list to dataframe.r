...
time.cm <- c(0,15,30,45,60,75,90,100,107,110,113)
...
########The Final Function df.make() Takes a list of vectors to be coerced into a data frame a list of 
#names to be used for the headers, and a parameter (either 'row' or 'col') that specifies whether the 
#data should be coerced into rows or columns

fermentation <- df.make(growth.list, name.list, 'col')
View(fermentation)
...
make.df <- function(datalist, colnamelist, version){
  
  if(version == 'col'){ #puts vectors as columns of data frame
  ...
  if(version == 'row'){
    dataframe <- data.frame(matrix( #Creating empty dataframe
      ...
      ncol=biggestsublist(datalist,'length')))
...
df.make <- function(datalist,colnamelist, version){
  if(version == 'col'){
  ...
  if(version == 'row'){
    length.data <- biggestsublist(datalist, 'length')
    length.names <- length(colnamelist)
  }
  ...
  if(length.data == length.names){  
    ...
    if( ! balanced.bool(datalist)) {
      ...
      make.df(balanced.list, colnamelist, version) #calls make.df to make the dataframe
    }  
    
    else{
      make.df(datalist,colnamelist, version)    
    }
  }
